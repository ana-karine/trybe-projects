...
function rgbInputText() {
  rgbText.innerHTML = `rgb${rgbGenerator()}`;
}
...
function rgbColorBalls() {
  ...
  balls[Math.floor(Math.random() * 5)].style.backgroundColor = `${rgbText.innerHTML}`;
}
...
function checkColor() {
  ...
  balls.forEach((ball) => {
    ball.addEventListener('click', () => {
      if (ball.style.backgroundColor === `${rgbText.innerHTML}`) {
